...
void Game::chooseStrength() {
	do{
		...
		if(rockStrength < 0)
		{
			cout << "Invalid Strength." << endl;
		}
	}while(rockStrength < 0);
	do{
	...
	}while(paperStrength < 0);
	do{
		cout << "Pick your strength for Scissors: ";
		cin >> scissorsStrength;
		cout << endl;
		if(scissorsStrength < 0)
		{
			cout << "Invalid Strength." << endl;
		}
	}while(scissorsStrength < 0);
}
...
int Game::GamePlay() {
	...
	while(ModStrength != 1 && ModStrength != 2) //data validation
	{
		cout << "1) Yes" << endl;
		cout << "2) No" << endl;
		cin >> ModStrength;
		if(ModStrength != 1 && ModStrength != 2)
		{
